...
int main() {
  // monitor::MonitorInter* monitor = new monitor:: xxx();

  // create a series of MonitorInter pointers - base class 
  std::vector<std::shared_ptr<monitor::MonitorInter>> runners_;

  /*
    std::shared_ptr<monitor::MonitorInter> cpu_load_monitor_ptr(new monitor::CpuLoadMonitor());
    runners_.emplace_back(cpuload_ptr);
  ...
  */
  ...
  // create rpc_client_ to send data 
  monitor::RpcClient rpc_client_;
  char *name = getenv("USER");


  std::unique_ptr<std::thread> thread_ = nullptr;
  // create  thread execute UpdateOnce function
